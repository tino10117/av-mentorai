...
Generá contenido que realmente venda, no genérico."""
    ...
    if herr_sel == "Plantillas":
            ...
            if plant_sel:
                ...
                st.markdown(f"""<div style="background:rgba(15,23,42,.95);border:1px solid rgba(250,204,21,.3);border-radius:16px;padding:20px;white-space:pre-wrap;font-family:monospace;font-size:13px;color:#f1f5f9;line-height:1.6">{contenido_planta}</div>""", unsafe_allow_html=True)
                st.code(contenido_planta, language=None)
                st.download_button(
            ...
            if st.button("🪄 Personalizar con IA", key="btn_personalizar") and mi_negocio_p.strip():
                ...
                with st.spinner("Personalizando..."):
                    try:
                        r = client.chat.completions.create(model="gpt-4o",
                            messages=[{"role":"system","content":"Sos un experto en marketing y ventas para LATAM. Adaptás plantillas de negocios de forma concreta y práctica."},
                                      {"role":"user","content":prompt_p}],
                            max_tokens=800)
                        resp_p = r.choices[0].message.content
                        st.session_state.plantilla_personalizada = resp_p
                        sumar_xp(10)
                    except Exception as e:
                        st.error(f"Error: {e}")

            if st.session_state.get("plantilla_personalizada"):
                st.markdown("**Plantilla personalizada:**")
